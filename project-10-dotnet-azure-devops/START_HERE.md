...
Open the README locally and map the three exercises into a paper or local notes checklist, starting with the ASP.NET Core repository path and pipeline objects without signing into or provisioning Azure.
...
2. 2. If a repository path is hard to locate in your notes, copy it exactly from the exercise and verify each folder name before interpreting the expected change
3. 3. If a trigger appears not to fire in the described flow, check whether the packet attributes it to a commit, a new build artifact, or an optional manual setting before diagnosing further.
