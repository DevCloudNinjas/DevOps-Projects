...
def run_validator(script: str, fixture: Path) -> None:
    ...
    if result.returncode:
        fail(
            f"{script} rejected {fixture.relative_to(ROOT)}: {result.stderr.strip() or result.stdout.strip()}"
        )


def main() -> int:
    subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "validate-l2-cloud-pilots.py")],
        cwd=ROOT,
        check=True,
    )
    for project, selected in PILOTS:
        ...
        if (
            selected
            and "sole selected pilot"
            not in (root / "pilot" / "README.md").read_text(encoding="utf-8").lower()
        ):
        ...
        if missing:
            fail(
                f"{project}: policy lacks required deny/control terms: {', '.join(missing)}"
            )
        fixture_dir = root / "fixtures" / "l2-source-contract"
        run_validator(
            "validate-l2-authorization.py", fixture_dir / "authorization-valid.json"
        )
        run_validator("validate-l2-evidence.py", fixture_dir / "evidence-valid.json")
        schema = json.loads(
            (root / "pilot" / "authorization-schema.json").read_text(encoding="utf-8")
        )
