# Catalog

The root [README](https://github.com/DevCloudNinjas/DevOps-Projects#readme) is the canonical project catalog. It groups the repository by cloud provider, difficulty, project type, learning tracks, and reference resources.

Use this docs section for cross-repository navigation:

- Start with the [project picker](project-picker.md) when choosing a lab by goal, tool, cloud, cost, or deployability.
- Use [learning paths](../learning-paths/index.md) when you want a sequenced route instead of a single project.
- Use projects 50-54 for modern, student-friendly 2026 home labs: GitOps, OpenTelemetry, OpenTofu, supply-chain security, and progressive delivery.
- Use [flagship docs](../flagship/index.md) for mature examples that combine multiple DevOps practices.
- Use [runbooks](../runbooks/index.md) before making broad infrastructure or Kubernetes changes.
- Use [security baselines](../security-baselines/index.md) as review checklists before opening a pull request.

When adding future docs, prefer short index pages here that point to existing project READMEs instead of duplicating project instructions.

## Fast Routes

| Student goal | Start here |
|---|---|
| I am new to DevOps | [Beginner path](../learning-paths/beginner.md) |
| I want containers and clusters | [Docker/Kubernetes path](../learning-paths/docker-kubernetes.md) |
| I want infrastructure as code | [Terraform/IaC path](../learning-paths/terraform-iac.md) |
| I want pipelines | [CI/CD path](../learning-paths/cicd.md) |
| I want security in delivery | [DevSecOps path](../learning-paths/devsecops.md) |
| I want AWS portfolio projects | [AWS path](../learning-paths/aws.md) |
| I want modern platform engineering | [Platform Engineering 2026 path](../learning-paths/platform-engineering-2026.md) |
