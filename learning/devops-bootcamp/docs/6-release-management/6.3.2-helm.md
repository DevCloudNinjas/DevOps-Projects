...
In this exercise we will take an existing Helm chart which deploys the [DevOps Knowledge Share API](https://github.com/devcloudninjas/dks-api).
...
1. Clone [dks-ui](https://github.com/devcloudninjas/dks-ui) if you haven't already. Run `make docker-build` and push the resulting image to your Docker Hub account.

2. Add `deployment-frontend.yaml`, `service-frontend.yaml`, and `configmap-frontend.yaml` files to the templates folder. Use the existing `service-backend.yaml`, `dks-global-config.yaml`, and `deployment-backeng.yaml` as reference. Your configmap will need to set the following [environment variables](https://github.com/devcloudninjas/dks-ui/blob/fed75825d4816504753883319e0c0a121f92fa51/.env.template)
