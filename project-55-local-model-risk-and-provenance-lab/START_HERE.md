...
From the project root, inspect README.md and the checked-in fixtures, then run only the permitted offline command `sh validate-p2-local.sh` with Python 3.8+ available.
...
2. 2. The local output reports the provenance hash and evaluation metric while showing PASS for the positive control
3. 3. The generated evidence/validator-result.json records both tampered-model-hash rejection and incomplete-approval rejection, alongside the command and fixture set.
...
2. 2. If the provenance check fails, compare the recorded model hash with the bytes of fixtures/model.bin rather than editing the result
3. 3. If a control decision fails, inspect the relevant risk-register.json, approval.json, and evaluation.json fields and check for missing, contradictory, or incomplete values.

## Evidence to capture

Local command transcript plus evidence/validator-result.json showing PASS, computed accuracy and model SHA-256, fixture set, and both negative-case checks
